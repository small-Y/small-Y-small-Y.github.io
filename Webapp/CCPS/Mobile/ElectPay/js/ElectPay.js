     ...
     + '</div>'
        ...
        getRoomInfo: function (opcpath) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    var OnLineFlag = (result.bOnLineFlag == true ? "在线" : "离线");
                    TUI.env.ElectPay.NodeID = result.NodeID
        ...
        getConsumeDetail: function (opcpath, year, month) {
            ...
            var lastDay1 = year + "-" + month + "-" + lastDay + " 23:59:59"
            console.log(firstDay);
            console.log(lastDay1);
            $.ajax({
                type: 'get',
                url: "/API/System/CCPS/Mining/Statistics/Day?NodeFullTag=" + opcpath + "&keyvalue=TotalCurrent&groupby=Time&startTime=" + firstDay + "&endTime=" + lastDay1,
                dataType: "json",
                error: function (result) {
                    $.toast("网络异常", "forbidden");
                },
                success: function (result) {
                    ...
                    for (var i = 0; i < result.data.length;i++){
                        $("#roomConsume").append(
                            ...
                            + '<p>' + result.data[i].day + '日</p>'
                            + '</div>'
                            + '<div class="weui_cell_ft"> 日用电：' + result.data[i].value.DayMeasure + '度</div>'
                            + '</div>'
        ...
        getChargeDetail: function (opcpath, startDate) {
            console.log(TUI.env.ElectPay.NodeID);
            $.showLoading();
            $.ajax({
                type: 'get',
                url: "/API/System/CCPS/Facility?opcpath=" + opcpath,
                dataType: "json",
                error: function (result) {
                    $.toast("网络异常", "forbidden");
                },
                success: function (result) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "/Project/CCPS/API/AccountOperate.tjs?accountID=" + NodeID + "&startTime=2018-05-27 11:09:24&operate_type=5&endTime=" + startDate,
                        dataType: "json",
                        error: function (result) {
                            $.toast("网络异常", "forbidden");
                        },
                        success: function (result) {
                            ...
                            for (var i = 0; i < result.data.length;i++){
                                $("#roomCharge").append(
                                    ' <div class="weui_media_box weui_media_text">'
                                    + '   <h4 class="weui_media_title">' + result.data[i].operate_name + '</h4>'
                                    + '   <p class="weui_media_desc"> ' + result.data[i].operate_time + ' ' + result.data[i].operate_tag + result.data[i].operate_info + '</p>'
                                    + '</div>'
                                );
                            }
                            $.hideLoading();
