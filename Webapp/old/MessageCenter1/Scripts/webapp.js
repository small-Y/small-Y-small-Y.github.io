   ...
   var handleHeight = function() {
    ...
    return {
		...
		load:function(){
			...
			//
			this.oTable1 = $('#messageListView').DataTable({
				"language": {
					"aria": {
				...
				"pageLength": 10,
				"ajax": 'srv/getMessageList.tjs?type=0'
			});
			//
			$('#messageListView').find('.group-checkable').change(function () {
			...
			$('#messageListView').on('change', 'tbody tr .checkboxes', function () {
		...
		sendMessage:function(title,content){
			bootbox.dialog({
								message: '<form class="form-horizontal" role="form">'
										...
										+'			<label class="col-sm-2 control-label" style="padding: 7px 0 0 0;">消息标题：</label>'
										+'			<div class="col-sm-10" style="padding-left: 0;"><input id="title" value="'+title+'" class="form-control" placeholder="请输入消息主题..."/></div>'
										+'		</div>'
										+'		<div class="form-group">'
										+'			<label class="col-sm-2 control-label" style="padding: 7px 0 0 0;">消息内容：</label>'
										+'			<div class="col-sm-10" style="padding-left: 0;"><textarea id="content" rows="3" class="form-control" placeholder="请输入消息内容...">'+content+'</textarea></div>'
										+'		</div>'
										+'	</div>'
										+'</form>',
								title: "发送消息",
								buttons: {
								  success: {
									...
									callback: function() { 
										...
										if(title=="")
										{
											toastr["warning"]("没有输入消息主题！","发送消息");
											return;
										}
										//
										if(content=="")
										{
											toastr["warning"]("没有输入消息内容！","发送消息");
											return;
										}
										//
										$.ajax({
													...
													success: function (result) {
														if(!result.flag)
														{
															toastr["error"](result.text,"发送消息失败");
															return;
														}
													}
											});
		...
		listMessage:function(type){
			...
			switch(type)
			{
				...
				case 1:
				{
					...
					$("#profile-title").html('<i class="fa fa-random font-green-sharp"></i><span class="caption-subject font-green-sharp bold uppercase">待发送消息</span>');
				...
				case 2:
				{
					...
					$("#tab_1_1").addClass("active");
					$("#profile-title").html('<i class="fa fa-thumbs-o-up font-green-sharp"></i><span class="caption-subject font-green-sharp bold uppercase">已发送消息</span>');
					$("#mainFrame").find(".btn-search").show();
					$("#mainFrame").find(".btn-edit").show();
					$("#mainFrame").find(".btn-prev").hide();
					$("#mainFrame").find(".btn-next").hide();
				...
				case 3:
				{
					...
					$("#tab_1_1").addClass("active");
					$("#profile-title").html('<i class="fa fa-thumbs-o-down font-green-sharp"></i><span class="caption-subject font-green-sharp bold uppercase">已废弃消息</span>');
					$("#mainFrame").find(".btn-search").show();
					$("#mainFrame").find(".btn-edit").show();
					$("#mainFrame").find(".btn-prev").hide();
					$("#mainFrame").find(".btn-next").hide();
				...
				case 4:
				{
					...
					$("#profile-title").html('<i class="fa fa-dashboard font-green-sharp"></i><span class="caption-subject font-green-sharp bold uppercase">消息中心视窗</span>');
					$("#mainFrame").find(".btn-search").hide();
					$("#mainFrame").find(".btn-edit").hide();
				...
				case 5:
				{
					...
					$("#tab_1_1").addClass("active");
					$("#profile-title").html('<i class="fa fa-inbox font-green-sharp"></i><span class="caption-subject font-green-sharp bold uppercase">已收到消息</span>');
					$("#mainFrame").find(".btn-search").show();
					$("#mainFrame").find(".btn-edit").show();
					$("#mainFrame").find(".btn-prev").hide();
					$("#mainFrame").find(".btn-next").hide();
		...
		loadMessageList:function(type){
			this.oTable1.ajax.url( 'srv/getMessageList.tjs?type='+type+'&startTime='+this.historyTime1.Format('yyyy-MM-dd')+'&endTime='+this.historyTime2.Format('yyyy-MM-dd')).load();
		},
		...
		deleteMessage:function()
		{
			...
			{
				toastr["error"]("请首先选中需要删除的消息！！","删除选中消息失败");
				return;
			}

			bootbox.dialog({
								message: '您确定要删除当前选中的'+messageList.length+'个消息？',
								title: "删除消息",
								buttons: {
								  success: {
									label: "确定",
									...
									callback: function() { 
										$.ajax({
													...
													data: {messageList:TUI.JSON.encode(messageList)},
													dataType: "json",
													context:this,
													error: function (result) {
														alert("远程服务故障，请检查网络或稍后再试！");
													...
													success: function (result) {
														if(!result.flag)
														{
															toastr["error"](result.text,"删除选中消息失败");
															return;
														}
														//
														switch(App.doType)
